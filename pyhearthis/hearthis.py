...
from .hearthis_requests import (
    AddToExistingPlaylistRequest,
    AddToNewPlaylistRequest,
...
class HearThis:
    ...
    @staticmethod
    def _feed_type_as_string(feed_type: FeedType) -> str:
        if feed_type is FeedType.NEW:
            return "new"
        if feed_type is FeedType.POPULAR:
            return "popular"

        return ""

    @staticmethod
    def _tracklist_type_as_string(artist_type: ArtistTracklistType) -> str:
        if artist_type is ArtistTracklistType.TRACKS:
            return "tracks"
        if artist_type is ArtistTracklistType.LIKES:
            return "likes"
        if artist_type is ArtistTracklistType.PLAYLISTS:
            return "playlists"

        return ""

    @staticmethod
    def _search_type_as_string(search_type: SearchType) -> str:
        if search_type is SearchType.TRACKS:
            return "tracks"
        if search_type is SearchType.USER:
            return "user"
        if search_type is SearchType.PLAYLISTS:
            return "playlists"
    ...
    async def _post_as_form_data(self,
                                 route,
                                 request,
    ...
    async def get_feeds(self,
                        ...
                        feed_end: date = None) -> List[SingleTrack]:
        ...
        duration_in_minutes = None if duration is None else round(
            duration.total_seconds() / 60)
        request = FeedRequest(user.key,
                              user.secret,
                              duration_in_minutes,
                              HearThis._feed_type_as_string(
                                  feed_type),
                              category,
                              start,
                              end,
    ...
    async def get_category_tracks(self,
                                  user: LoggedinUser,
                                  category: Category,
                                  page: int = 1,
                                  count: int = 5) -> List[SingleTrack]:
        assert count <= 20, 'maximum allowed pagecount is 20'
    ...
    # pylint: disable=too-many-arguments
    async def get_artist_tracks(self,
                                user: LoggedinUser,
                                user_permalink: str,
                                track_type: ArtistTracklistType = ArtistTracklistType.TRACKS,
                                page: int = 1,
                                count: int = 5) -> List[SingleTrack]:
        assert count <= 20, 'maximum allowed pagecount is 20'
        ...
        json_data = await self._get_as_json(route,
                                            ArtistTracksRequest(user.key,
                                                                user.secret,
                                                                HearThis._tracklist_type_as_string(
                                                                    track_type), page, count
                                                                )
                                            )
        return list(map(HearThis._json_to_track, cast_list(json_data)))
    # enable: disable=too-many-arguments
    ...
    async def create_playlist(self,
                              user: LoggedinUser,
                              playlist_name: str,
                              private_set: bool = True
                              ) -> None:
        route = "set_ajax_add.php"
        privat = 1 if private_set else 0
        await self._post_as_form_data(route,
                                      AddPlaylistRequest(user.key,
                                                         user.secret,
                                                         playlist_name,
                                                         privat=privat),
                                      200
    ...
    async def add_track_to_playlist(self,
                                    user: LoggedinUser,
                                    track: SingleTrack,
                                    playlist: Playlist
                                    ) -> Playlist:
        route = "set_ajax_add.php"
        json_str = await self._post_as_form_data(route,
                                                 AddToExistingPlaylistRequest(user.key,
                                                                              user.secret,
                                                                              track.id,
                                                                              playlist.id),
                                                 200
    ...
    async def add_track_to_new_playlist(self,
                                        user: LoggedinUser,
                                        track: SingleTrack,
                                        playlist_name: str):
        route = "set_ajax_add.php"
        json_str = await self._post_as_form_data(route,
                                                 AddToNewPlaylistRequest(user.key,
                                                                         user.secret,
                                                                         track.id,
                                                                         playlist_name
                                                                         ),
    ...
    async def get_playlist_tracks(self,
                                  user: LoggedinUser,
                                  playlist: Playlist) -> List[SingleTrack]:
        route = f"set/{playlist.permalink}/"
        json_data = await self._get_as_text(route,
                                            CredentialsRequest(
    ...
    async def delete_track_from_playlist(self,
                                         user: LoggedinUser,
                                         track: SingleTrack,
                                         playlist: Playlist) -> Playlist:
        route = "set_ajax_add.php"
        json_str = await self._post_as_form_data(route,
                                                 DeleteFromPlaylistRequest(user.key,
                                                                           user.secret,
                                                                           track.id,
                                                                           playlist.id
                                                                           ),
    ...
    async def delete_playlist(self, user: LoggedinUser, playlist: Playlist) -> None:
        ...
        response = await self._post_as_form_data(route,
                                                 DeletePlaylistRequest(user.key,
                                                                       user.secret,
                                                                       playlist.id),
                                                 200
                                                 )
    ...
    async def search(self, user: LoggedinUser,
                     query: str,
        ...
        duration_in_minutes = None if duration is None else round(
            duration.total_seconds() / 60)
        current_type = None if search_type is None else self._search_type_as_string(
            search_type)

        request = SearchRequest(
            user.key, user.secret, query, current_type, duration_in_minutes, page, count)
        json_data = await self._get_as_json(route, request)

        return list(map(HearThis._json_to_track, cast_list(json_data)))

    # pylint: disable=unused-argument
    async def reload_single_track(self,
                                  user: LoggedinUser,
                                  track: SingleTrack
                                  ) -> SingleTrack:
